...
import numpy as np
from Layout_Service.graph.CNNGlobalLayoutModel import GlobalVgg16Cnn
from Layout_Service.utils.transformer import embedding, multihead_attention, feedforward, normalize, \
    denseWithL2loss, featureGen_DarkNet, calcLabelSimlarity
from Layout_Service.utils.Hyper_parameter import Hyperparams as hp
from Layout_Service.utils.show_img import show_img, show_state
from checkCrash.CrashChecker import CrashChecker
from datetime import datetime

from Layout_Service.utils.transformer import feature_cnn_pix, create_conv_Unet

configpath = ["../checkCrash/config.props", "./checkCrash/config.props", "config.props","F:\pycharm\WorkSpace\GA\GA_functionZone\checkCrash\config.props"]

configpath = [x for x in configpath if os.path.exists(x)][0]
...
class LayoutTransformer_vgg(GlobalVgg16Cnn):
    ...
    def __init__(self, image_width: int, image_height: int, furniture_nums: int, label_size: int,
        ...
        self.use_furniture_cnn = False
        self.use_fill_furniture = True
        self.use_cnn_predict_encoder = False
        self.use_cnn_predict_predict = False
        ...
        self.count_info["crash_num"] = 0

        self.use_cnn_pix = False
        self.use_U_net = False
        self.batch_size = self.max_length
        self.loadCrashChecker(configPath=configPath)

        self.save_log = False
        self.resort_label = False
        ...
        with tf.variable_scope("get_data", reuse=tf.AUTO_REUSE):
            if self.use_cnn:
                ...
                if self.use_cnn_pix:
                    self.room_context = tf.reshape(self.room_context,
                                                   shape=[self.batch_size, self.image_height, self.image_width,
                                                          self.ebd_size])
                    self.room_context = feature_cnn_pix(self.room_context, scope="room_cnn_pix", reuse=tf.AUTO_REUSE,
                                                        nr_resnet=1, nr_filters=10, nr_logistic_mix=1)
                    shape_roomt_cnn = self.room_context.shape.as_list()
                    self.room_context = tf.reshape(self.room_context,
                    ...
                    self.room_context = tf.reshape(self.room_context,
                                                   shape=[self.batch_size, self.image_height, self.image_width,
                                                          self.ebd_size])
                    self.room_context, _, change_size = create_conv_Unet(self.room_context, keep_prob=1,
                                                                         n_class=self.ebd_size, layers=3,
                                                                         features_root=16,
                                                                         filter_size=3, pool_size=2,
                                                                         channels=self.ebd_size,
                                                                         scope="room_context_uNet")
                ...
                self.furniture_fea = tf.layers.dropout(self.furniture_fea, rate=self.dropout_rate,
                                                       training=tf.convert_to_tensor(trainable))
                if self.use_cnn_pix:
                    self.furniture_fea = tf.reshape(self.furniture_fea,
                                                    shape=[self.batch_size, self.image_height, self.image_width,
                                                           self.ebd_size])
                    self.furniture_fea = feature_cnn_pix(self.furniture_fea, scope="furniture_fea", reuse=tf.AUTO_REUSE,
                                                         nr_resnet=1, nr_filters=10, nr_logistic_mix=1)
                    sh_furniture_fea = self.furniture_fea.shape.as_list()
                    self.furniture_fea = tf.reshape(self.room_context,
                                                    shape=[-1, self.image_height, self.image_width,
                                                           sh_furniture_fea[-1]])
                elif self.use_U_net:
                    self.furniture_fea = tf.reshape(self.furniture_fea,
                                                    shape=[self.batch_size, self.image_height, self.image_width,
                                                           self.ebd_size])
                    self.furniture_fea, _, change_size = create_conv_Unet(self.furniture_fea, keep_prob=1,
                                                                          n_class=self.ebd_size, layers=3,
                                                                          features_root=16,
                                                                          filter_size=3, pool_size=2,
                                                                          channels=self.ebd_size,
                                                                          scope="furniture_fea_uNet")
    ...
    def build_attention_graph(self, trainable: bool = True):
        ...
        if self.use_cnn:
            ...
            self.build_attention_graph_tensor(
                self.target, room=self.room_zid, furniture=self.zone_zid, trainable=trainable)

    def predict_route(self, room_json: str, top_k: int, room_type_id: int, save_log: bool = False):
        pred, pred_topk = self.hidden_predict_func(roomJson=room_json, top_k=top_k, roomTypeId=room_type_id,
                                                   )
        if save_log:
            self.save_log = save_log
            self.png_path = "/ai/data/log/pngs"
            if os.path.exists(self.png_path):
                pass
            else:
                print("log pngs path not exists")
                os.makedirs(self.png_path)

        pred_topk_dst = []
        print(pred_topk, pred)
        ...
        # if pred not in pred_topk_dst:
        #     pred_topk_dst.insert(0, pred)
    ...
    def hidden_predict_func(self, roomJson, top_k, roomTypeId=1):
        ...
        for i in range(max_length):
            if i < len(ck.furniture_mats):
                furniture_mats.append(ck.furniture_mats[i][0].tolist())
            else:
                furniture_mats.append(np.zeros(shape=shp).tolist())
        ...
        length_no_zero = len([x for x in data_infos if x["zid"] not in [99, 98]])
        pred_ = []
        try:
            for furniture_id in range(length_no_zero):
                ...
                #     print("input:", length_no_zero)
                #
                #     print(temp_targets)
                #     print(len(temp_all_ready_furnitures))
                #
                #     for ix, x in enumerate(temp_all_ready_furnitures):
                #         #                         print("验证中间状态:",ix)
                #         #                         print(preds)
                #
                #         show_img((furniture_mats * top_k)[ix], title="No.{} Zones".format(ix))
                ...
                feed_dict = {
                    self.zone_zid: furniture_mats * top_k,
                    self.room_zid: temp_all_ready_furnitures,
                    self.target: temp_targets,
                }
                pred, logits = self.sess.run([self.pred, self.logits], feed_dict=feed_dict)
                if furniture_id == 0:
                    preds = []
                    scores = []
                    out_ids = []
                ...
                if furniture_id < length_no_zero:
                    if isinstance(preds, list):
                        ...
                        preds = preds.tolist()
                preds, scores, out_ids, mid_states = self.get_mid_beamsearch_add_check_Crash(temp_log, top_k,
                                                                                             furniture_id,
                                                                                             preds, scores,
                                                                                             max_length=max_length,
    ...
    def get_mid_beamsearch_add_check_Crash(self, logits, topk, i, preds=[], scores=[], \
        ...
        if i == start_ind:
            ...
            for j in range(topk_new):
                _in_inndexs.append([0])
                _out_indexs.append([0])
            ...
            if self.resort_label:
                n_score = []
                for score_i, score in enumerate(_scores):
                    s_c = []
                    n = 0
                    for score_i_ in range(0, score_i):
                        n += 1
                        s_c.append(
                            score * (1 - calcLabelSimlarity(int(_pred[score_i_][i]), int(_pred[score_i][i]))))
                    if len(s_c) > 0:
                        s_c = sum(s_c) / n
                    else:
                        s_c = score
            ...
            if self.resort_label:
                ...
                for score_i, score in enumerate(_scores):
                    ...
                    for score_i_ in range(0, score_i):
                        n += 1
                        s_c.append(score * (1 - calcLabelSimlarity(int(_pred[score_i_][i]), int(_pred[score_i][i]))))
                    if len(s_c) > 0:
                        s_c = sum(s_c) / n
                    else:
                        s_c = score
                ...
                if debug:
                    print(("第{0}次,_pred:{1},scores:{2},_in_indexs,_mid_states{3}".format(i, _pred, _scores,
                                                                                         _in_indexs, )))
            else:
                    ...
                    if ret == 0:
                        ...
                        if self.save_log:
                            show_state(mask_data[0],
                                       "time_{0} NO{1} Zones ".format(str(datetime.now())[:20], i),
        ...
        if len(preds) == 0:
            ...
            if len(ori_preds) == 0:
                for top in range(topk):
                    ...
                    n_scores.append(0)
                    n_out_ids.append(0)
                    n_out_states.append(ori_out_states[top])
            ...
            if i == max_length - 1:
                if add_check_Crash:
                    self.clash_checker.finish_step()
            preds = n_preds
            scores = n_scores
            out_ids = n_out_ids
            states = n_out_states
        ...
        self.count_info["crash_num"] += ck_num
