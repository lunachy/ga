...
#
# def int_shape(x):
#     return x.shape.as_list  # list(map(int, x.get_shape()))
#
#
# def concat_elu(x):
...
#     xs = x.shape.as_list()  # true image (i.e. labels) to regress to, e.g. (B,32,32,3)
#     ls = l.shape.as_list()  # predicted distribution, e.g. (B,32,32,100)
...
#     log_probs = tf.where(x < -0.999, log_cdf_plus, tf.where(x > 0.999, log_one_minus_cdf_min, tf.where(cdf_delta > 1e-5,
#                                                                                                        tf.log(
#                                                                                                            tf.maximum(
#                                                                                                                cdf_delta,
#                                                                                                                1e-12)),
...
#
#             hw = get_var_maybe_avg('hw', ema, shape=[h.shape.as_list()[-1], 2 * num_filters], dtype=tf.float32,
#                                    initializer=tf.random_normal_initializer(0, 0.05), trainable=True)
#         if init:
...
# def down_shift(x):
#     xs = x.shape.as_list()
#     return tf.concat([tf.zeros_like(x[:, 0:1, :, :]), x[:, :xs[1] - 1, :, :]], 1)
#
#
# def right_shift(x):
#     xs = x.shape.as_list()
#     return tf.concat([tf.zeros_like(x[:, :, 0:1, :]), x[:, :, :xs[2] - 1, :]], 2)
#
#
# @add_arg_scope
