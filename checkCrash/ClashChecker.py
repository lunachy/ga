...
if platform == "linux":
    ...
    paths = ["ConsoleApplication1.dll", "checkCrash/ConsoleApplication1.dll", "../checkCrash/ConsoleApplication1.dll"]
...
class CrashChecker:
    ...
    def init_room(self, roomTypeId, room_str):
        ...
        self.room_length = get_roomLen_from_config(self.configPath, roomTypeId=int(roomTypeId))
    ...
    def convert2real(self, label, z=0, cid=0):
        ...
        self.max_room_size = 6400  # self.room_length
...
def get_conf_from_config(configPath, _key):
    with open(configPath, encoding='utf-8') as f:
        for line in f:
            if line.find(_key) != -1:
                splits = line.split('=')
                assert len(splits) == 2, 'no config!'
                return int(splits[1].strip())
...
def get_roomLen_from_config(configPath, roomTypeId):
    with open(configPath, 'r', encoding='utf-8') as f:
        for line in f:
            if line.find("room_metadatas") != -1:
                ...
                segNumStr = splits[1].strip()
                configs = segNumStr.split(",")
                for config in configs:
                    k_str, v_str = config.split(":")
                    if int(k_str.strip()) == roomTypeId:
                        return int(v_str)
